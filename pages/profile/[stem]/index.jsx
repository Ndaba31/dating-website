	...
	if (status === 'authenticated') {
		...
		useEffect(() => {
			...
			const getPumpkin = async () => {
				...
				const res = await fetch(
					`${process.env.NEXT_PUBLIC_URL}/api/pumkins/${id}`,
					getData
				);
				const {
					user,
		...
		return (
			<>
										...
										{isSlider ? (
											...
											<div id='choice'>
												<p style={{ fontSize: '12pt' }}>
													<b>
														This Person has already initiated to match
														with you{' '}
													</b>
												</p>
				...
				{hickies.filter(({ stem }) => stem !== currentUser.stem).length !== 0 && (
